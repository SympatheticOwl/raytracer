...
void ray_sphere_intersect(ray_result *res,
                          double ray[2][3],
                          const double sphere[4]) {
    const double radius = sphere[3];
    const double *ray_origin = ray[0];
    const double *ray_dir = ray[1];

    double temp[3];
    vec3_subtract(temp, sphere, ray_origin);
    const double c_minus_ro_squared = vec3_squared_length(temp);
    const double radius_squared = radius * radius;
    const bool inside = c_minus_ro_squared < radius_squared;

    const double tc = vec3_dot(temp, ray_dir);
    ...
    vec3_scale_add(closest_point, ray_origin, ray_dir, tc);
    ...
    const double t_offset = sqrtf(radius_squared - d_squared);
    ...
    res->hit = true;
    res->distance = t;
    vec3_scale_add(res->point, ray_origin, ray_dir, t);
...
void ray_plane_intersect(ray_result *res,
    ...
    double d = plane[3]; // misussing radius from vec struct as d

    const double ray_dir_dot_normal = vec3_dot(ray_dir, normal);

    if (fabs(ray_dir_dot_normal) < EPSILON) {
        return;
    }
    ...
    double temp[3];
    vec3_subtract(temp, point_on_plane, ray_origin);

    const double t = vec3_dot(temp, normal) / ray_dir_dot_normal;
...
void ray_intersection(ray_result *res,
    ...
    // type 1 is "sun" which is unused here
    if (v->type == 0) {
...
bool check_shadow(const double origin[3],
            ...
            if (hit_anything) {
                ...
                for (int j = 0; j < suns->vector_count; j++) {
                    ...
                    if (normal_dot_light > 0) {
                        ...
                        bool in_shadow = check_shadow(shadow_origin, light_dir, INFINITY, objects);
